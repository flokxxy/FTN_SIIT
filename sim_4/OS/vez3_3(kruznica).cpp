...
class Circle {
	...
	double r;
public:
	Circle(Point p0, double r) : p0(p0), r(r) {}
	double circle(const Point& t) {
		return abs(sqrt(pow(p0.x - t.x, 2) + pow(p0.y - t.y, 2)) - r);
	}
	void find_closest(VPci p_begin, VPci p_end, Closest_point& cpoint) {
		for(; p_begin != p_end; ++p_begin ) {
			cpoint.test_and_set(*p_begin, circle(*p_begin));
		}
	}
...
int main() {
	double x_centr = 0, y_centr = 0;
	double radius = 3;
	double random_x, random_y;

	Circle k({ x_centr,y_centr }, radius);
	vector<Point> p(10000);
	srand(time(NULL));

	for (int i = 0; i < 1000; i++) {
		random_x = rand() % (2 * (int)radius) + radius / 2;
		random_y = rand() % (2 * (int)radius) + radius / 2;
